/**
 * lazy-tools — hide all tools behind a manifest-aware tool_search.
 *
 ...
 *  "lazyTools": { "alwaysEnabled": ["lsp", "grep"], "showStatus": true }
 */
...
function readUserConfig(): UserConfig {
  try {
    ...
    const s = JSON.parse(raw)?.lazyTools ?? {};
    return {
...
export default function lazyToolsExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    ctx.ui.notify(
      `lazy-tools: ${manifest.length} tools hidden behind tool_search`,
      "info",
    );
  });
  ...
  pi.on("before_agent_start", (_event, ctx) => {
    ...
    if (showStatus) {
      ctx.ui.setStatus("lazy-tools", `${unlocked.size} / ${manifest.length + 1} tools`);
    }
  });
